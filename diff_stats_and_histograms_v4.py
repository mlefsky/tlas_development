...
#                print(str(",").join(output))
    ...
    #            print("aff",match,af['filename'])
     ...
     #           #print ("diff_"+file_list[ix-1],np.mean(rtmp2),min(rtmp2),max(rtmp2),np.std(rtmp2))
                p=1
                xrange=(np.percentile(rtmp2,p),np.percentile(rtmp2,100-(2*p)))
#                    xrange=()
 ...
 #                   sys.exit
                ...
                if (af['shot_density'] == "8") and (baseline_shot_density[[match_ix][0]])=="22":
                    print(np.mean(rtmp2),np.std(rtmp2),np.min(rtmp2),np.max(rtmp2))
                    sns.displot(data=pd.DataFrame(rtmp2, columns=['Difference']), x="Difference") 
                    plt.show()
                    sys.exit                    

#                    plt.hist(rtmp2,bins=40,density=True,range=xrange,kind="kde")#,weights=np.ones(len(rtmp2)) / len(rtmp2))
#                    plt.title(str(np.mean(rtmp2))+" "+str(np.std(rtmp2))+" "+str(np.min(rtmp2))+" "+str(np.max(rtmp2))+" "+str(xrange)+str(titles[ix-1]),fontsize=10)
                    ...
                    plt.title("Difference betwen 8 ppm and 22 ppm estimates of "+title,fontsize=10)
                    plt.xlabel('Difference',fontsize=10)          
                    plt.ylabel('Percent of observations',fontsize=10)   
                    plt.gca().yaxis.set_major_formatter(PercentFormatter(1))
                    plt.savefig('/home/ubuntu/density_comparison_v2/diffig_'+af['var']+".png")
                    print("savefig")
